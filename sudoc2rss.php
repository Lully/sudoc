...
$url = substr($current_url, strpos($current_url, "rss.php?")+12); 
...
echo '<channel>';
echo '<title>RSS - Recherche Sudoc</title>';
echo '<description>Recherche Sudoc</description>';
echo '<link>' . str_replace("&", "&amp;", $url) . '</link>';
  ...
  foreach($source ->find('input[name]') as $element) 
    {
	   ...
	   $ppn = $element->getAttribute('value'); 
	   if (strpos($name,'ppn') !== false) 
	     {
		   $url_rdf = "http://www.sudoc.fr/" . $ppn . ".rdf";
		   #$rdf_record = file_get_html($url_ppn);
		   $rdf_record = new DOMDocument();
		   $rdf_record->load($url_rdf);
		   
		   $xpath_dc = new DomXpath($rdf_record);
		   $xpath_dc->registerNamespace("dc", "http://purl.org/dc/elements/1.1");
		   
		   $xpath_rdf = new DomXpath($rdf_record);
		   $xpath_rdf->registerNamespace("rdf", "http://www.w3.org/1999/02/22-rdf-syntax-ns#");
		   
		   $title = $rdf_record->getElementsByTagName('title')->item(0)->nodeValue;

		   
		   $publisher = $rdf_record->getElementsByTagName('publisher')->item(0)->nodeValue;

		   echo "<item><title>";
		   echo $title;
		   echo "</title>";
		   echo "<link>http://www.sudoc.fr/" . $ppn . "</link>";
		   echo "<description>" . $publisher . "</description>";
		   echo"</item>";
	    }
    }	
 }
